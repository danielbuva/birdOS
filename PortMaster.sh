...
# MUOS_BESPOKE_NETWORK_ON_DEMAND_V1
# Boot keeps the Wi-Fi hardware powered down. PortMaster is one of the few
# applications that needs it, so establish the saved muOS connection here.
if ! ip route 2>/dev/null | grep -q '^default '; then
  echo "Starting Wi-Fi for PortMaster..." > "$CUR_TTY"
