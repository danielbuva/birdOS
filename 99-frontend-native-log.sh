...
EARLY_LAUNCHER_STATE="$LAUNCHER_ROOT/early-launcher-v5-system-actions.state"
...
if [ -s "$LAUNCHER_OBJECT" ] && [ -s "$EARLY_INIT_SOURCE" ] && [ ! -f "$EARLY_LAUNCHER_STATE" ]; then
	...
	if /usr/bin/ld -static --build-id=none -z noexecstack -s -e _start \
		-o "$LAUNCHER_NEW" "$LAUNCHER_OBJECT" >"$LAUNCHER_ROOT/link-early-v5-system-actions.log" 2>&1; then
		chmod 755 "$LAUNCHER_NEW"
		...
		if [ "$STARTUP_READY" -eq 1 ]; then
			...
			printf '%s native shutdown and on-demand PortMaster installed; active next boot\n' \
				"$(date -Iseconds 2>/dev/null || date)" >>"$BESPOKE_ROOT/install.log"
		else
