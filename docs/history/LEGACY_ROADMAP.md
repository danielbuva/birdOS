# Historical birdOS legacy roadmap

**Status:** this is the superseded feature-by-feature roadmap and accumulated
stage record formerly kept at the repository root. Statements below such as
"active," "current" and "next" describe their original development stage, not
the supported v6.23 build. The authoritative implementation and plan are
[`ACTIVE_PATH.md`](../../ACTIVE_PATH.md) and [`ROADMAP.md`](../../ROADMAP.md).
...
Stock-root v6.23 now hardens the proven software baseline established and
physically gated through v6.22:
...
- transactional automatic return to the accepted v5.4 clean root after the
  fixed failed-start threshold.
...
V6.23 is the repository-hardening candidate. It makes deployment and automatic
fallback transactional, derives preflight/copy/verification from one complete
manifest, makes application readiness fail closed, supervises launcher startup
and content process lifetime, and closes the reviewed persistence/error-reporting
edges. V6.22 remains the accepted physical checkpoint until this combined
candidate passes the same broad hardware gate.
...
- [x] Promote the launcher to a permanent shell. B navigates nested views; the
  dedicated main-page reload action only restarts Bird and is not the boot
  fallback. Keep
...
16. [active: v6.23; last physical gate: v6.22] Audit and reduce the retained
    ROCKNIX userspace contract;
    ...
    contract and replaces the crashing MSX core; v6.23 hardens deployment,
    fallback, readiness, launcher health, content containment and persistence;
    Wi-Fi profile capture is deferred
