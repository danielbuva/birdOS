# birdOS project chronology

This file preserves the measured results and version-by-version engineering
record formerly kept in the root README. It is historical evidence, not the
active implementation authority. See [`ACTIVE_PATH.md`](../../ACTIVE_PATH.md)
for the current build and runtime graph.

## Engineering measurements (chronological)
...
- Clean-root checkpoint LED-on stopwatch result: approximately 2.5 seconds. V5.2
...
## Implementation chronology through stock-root v6.23

- Stock-root v6.23 is the active repository candidate; v6.22 is its last
  hardware-tested predecessor. V6.3 established the coherent
  ...
  before the mount moved. V6.17 proved its guarded fallback but also proved
  that late bind aliases below an already-open `/run` directory were not
  discoverable by this old-root process: it retired at 4.45 seconds, the normal
  launcher recovered at 7.12 seconds and a redundant autostart UI request
  caused another repaint near 10 seconds. V6.18 removes the aliases and signals
  ...
  V6.23 is the repository-hardening successor: one complete manifest drives a
  versioned transactional deployment, fallback selection is atomic,
  application readiness fails closed, launcher startup failures receive
  bounded local recovery, and each foreground provider is contained in a
  systemd scope. It also closes the detached-exec, path-length, auxiliary-poll
  and shutdown-checkpoint failure paths found in review. Its broad physical
  gate is still required before it replaces v6.22 as the accepted device
  checkpoint.
...
functionality checkpoint remains in Git. At the clean-root checkpoint, v5.3
and the accepted v5.2, v5.1, v5.0 and v4.5 kernels were preserved on p6. That
checkpoint has since been superseded by the versioned stock-root path in
[`ACTIVE_PATH.md`](../../ACTIVE_PATH.md).

## Historical font payload
