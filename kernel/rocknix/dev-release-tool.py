...
def early_kernel_authority(manifest: Manifest) -> str:
    ...
    if (
        joypad_digest == SOURCE_PARITY_JOYPAD_SHA256
        and source_authorities.get("source-kernel-single-gpio-read.tsv")
        == SOURCE_SINGLE_GPIO_READ_AUTHORITY_SHA256
    ):
        return "source-single-gpio-read"
    if (
        joypad_digest == SOURCE_PARITY_JOYPAD_SHA256
        and source_authorities.get("source-kernel-single-input-sync.tsv")
    ...
    if (
        joypad_digest == SOURCE_PARITY_JOYPAD_SHA256
        and source_authorities.get("source-kernel-fixed-gpio-fastpath.tsv")
