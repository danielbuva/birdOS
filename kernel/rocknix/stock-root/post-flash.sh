...
verify_release_runtime() {
	...
	awk -F '\t' -v expected="$BIRD_RELEASE" '
		BEGIN {
			...
			optional_input["source-kernel-parity.tsv"] = 1
			optional_input["source-kernel-builtin-input.tsv"] = 1
			optional_input["source-kernel-single-gpio-read.tsv"] = 1
			optional_input["source-kernel-single-input-sync.tsv"] = 1
			optional_input["source-kernel-changed-input-sync.tsv"] = 1
			...
			optional_input["source-kernel-irq-buttons-no-raid6-deferred-wifi-lz4.tsv"] = 1
		}
		$1 == "schema" {
		...
		$1 == "input" {
			...
			if (!($2 in required_input) && !($2 in optional_input))
				exit 1
			if (input_seen[$2]++) exit 1
			if ($2 in optional_input) optional_inputs++
			inputs++
			next
		}
		$1 == "dir" {
		...
		END {
			...
			if (schema != 1 || release != 1 || policy != 1 || source != 1 ||
			    ...
			    inputs != 14 + optional_inputs || optional_inputs > 1 ||
