	...
	awk -F '\t' -v expected="$BIRD_RELEASE" '
		...
		$1 == "input" {
			...
			if (!($2 in required_input) && $2 != "source-kernel-parity.tsv" &&
			    ...
			    $2 != "source-kernel-fixed-input.tsv")
				exit 1
		...
		END {
			...
			if (schema != 1 || release != 1 || policy != 1 || source != 1 ||
			    ...
			    inputs != 14 + (input_seen["source-kernel-parity.tsv"] == 1) + (input_seen["source-kernel-builtin-input.tsv"] == 1) + (input_seen["source-kernel-fixed-input.tsv"] == 1) ||
			    input_seen["source-kernel-parity.tsv"] + input_seen["source-kernel-builtin-input.tsv"] + input_seen["source-kernel-fixed-input.tsv"] > 1 ||
