...
# reversible runtime decision until its separate product gate.
for UNIT in hdmi-hotplug.path video.service sixaxis@.service \
	systemd-rfkill.socket; do
	mount --bind /dev/null "/sysroot/usr/lib/systemd/system/$UNIT" || {
		error bird-deferred-hardware-mask "Could not mask $UNIT"
		return 1
	}
done
