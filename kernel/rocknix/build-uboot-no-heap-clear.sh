#!/bin/sh
# Reproduce the RG34XX-SP no-heap-clear U-Boot candidate twice.  This host
# artifact builder has no deployment mode and never opens removable media.
...
OUTPUT=${OUTPUT:-$ROOT/kernel/work/bird-uboot-no-heap-clear-20260701}
BASE=${BASE:-$ROOT/kernel/work/bird-uboot-direct-extlinux-20260701}
BL31=${BL31:-$ROOT/kernel/work/rocknix-system-exact-20260701/usr/share/bootloader/bl31.bin}
TOOLCHAIN_AUTHORITY=${TOOLCHAIN_AUTHORITY:-$ROOT/kernel/work/bird-uboot-green-20260701/toolchain-authority.tsv}
...
BASE_AUTHORITY_SHA=8f15f4e647f1aecd6266df3861aeb5a2534db26a3b6280801d0bd4efbd99ce09
...
DIRECT_VERIFIER="$ROOT/kernel/rocknix/verify-uboot-direct-extlinux-build.py"
...
for INPUT in "$FIT_TIMESTAMP_PATCHER" "$ENV_TRANSFORM" "$DIRECT_TRANSFORM" \
	"$NO_CLEAR_TRANSFORM" "$VERIFIER" "$DIRECT_VERIFIER"; do
	require_file "$INPUT"
done

"$PYTHON" "$DIRECT_VERIFIER" --verify-output "$BASE" >/dev/null

TOOLCHAIN="$ROCKNIX_BUILD/toolchain"
[ -d "$TOOLCHAIN" ] && [ ! -L "$TOOLCHAIN" ] || \
	fail "safe pinned toolchain directory missing: $TOOLCHAIN"

# Bind every executable and compiler-internal byte to the independently
# retained toolchain authority before the read-only container mount is used.
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/bird-uboot-no-clear.XXXXXX")
cleanup() {
	rm -rf "$WORK"
}
...
case "$1" in no-clear-a|no-clear-b) ;; *) exit 2 ;; esac
...
cp /input/no-clear.defconfig "configs/'"$DEFCONFIG"'"
TOOLCHAIN=/work/build.ROCKNIX-H700.aarch64/toolchain
...
grep -Fqx '\''CONFIG_BOOTCOMMAND="mmc dev 0; sysboot mmc 0:1 any ${scriptaddr} /extlinux/extlinux.conf"'\'' .config
...
for BUILD_NAME in no-clear-a no-clear-b; do
...
"$PYTHON" "$VERIFIER" \
	--build-a "$WORK/output/no-clear-a" \
	--build-b "$WORK/output/no-clear-b" \
	--base-authority "$BASE" \
	--bl31 "$BL31" \
...
printf 'Verified non-deploying no-heap-clear U-Boot candidate built:\n  %s\n' "$OUTPUT"
