#!/bin/sh
# Reproduce and seal the RG34XX-SP fixed-command-closure U-Boot candidate twice.
...
OUTPUT=${OUTPUT:-$ROOT/kernel/work/bird-uboot-fixed-command-closure-20260829}
...
VERIFIER=$ROOT/kernel/rocknix/verify-uboot-fixed-command-closure-build.py
...
require_hash "$FIXED_TRANSFORM" a70b73b0feb314f3486f32b9b2509af9a86c55a75eccb4a946c5c3ec4e767926
require_hash "$COMMAND_TRANSFORM" 25914a1d4209a584df8c045d2dfa3968bae7ced42f00f3be5ce5ae29837c7cd8
...
require_hash "$VERIFIER" 51e91beb96aec65b32369bd815fb9b3e4afa3954644303082ed891ade59fc38b
...
WORK=$(mktemp -d "$OUTPUT_PARENT/.bird-uboot-fixed-command-closure.XXXXXX") || fail 'could not create staging directory'
cleanup() {
	case "$WORK" in "$OUTPUT_PARENT"/.bird-uboot-fixed-command-closure.*) rm -rf -- "$WORK" ;; *) fail 'unsafe staging cleanup' ;; esac
}
...
require_hash "$WORK/input/fixed.defconfig" 84a062534a8b61012be20e299b5817374670ac17f54e37f0e5c4c075cb3ee26c
"$PYTHON" "$COMMAND_TRANSFORM" "$WORK/input/fixed.defconfig" "$WORK/input/commands.defconfig"
require_hash "$WORK/input/commands.defconfig" f0fdbcf89aa47e4ec4372c63fc0bd7457d4c4f42a04476e7489a6ffb9245074a
...
grep -Fqx "CONFIG_CMD_BOOTI=y" .config
...
grep -Fqx "CONFIG_DOS_PARTITION=y" .config
grep -Fqx "# CONFIG_FAT_WRITE is not set" .config
grep -Fqx "# CONFIG_FS_EXT4 is not set" .config
grep -Fqx "# CONFIG_EFI_PARTITION is not set" .config
grep -Fqx "# CONFIG_CMD_BOOTM is not set" .config
grep -Fqx "# CONFIG_CMD_MEMORY is not set" .config
...
for PASS in a b; do
	printf 'Building isolated fixed-command-closure pass: %s\n' "$PASS"
...
cleanup
printf 'Verified non-deploying fixed-command-closure U-Boot authority: %s\n' "$OUTPUT"
