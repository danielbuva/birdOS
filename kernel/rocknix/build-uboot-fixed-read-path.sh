#!/bin/sh
# Reproduce and seal the RG34XX-SP fixed-read-path U-Boot candidate twice.
...
OUTPUT=${OUTPUT:-$ROOT/kernel/work/bird-uboot-fixed-read-path-20260829}
...
require_hash "$VERIFIER" cd9e305e483fa898ebca49d0760e5cc3abee22c7f2acc049c1d96fd9d11c6e13
...
WORK=$(mktemp -d "$OUTPUT_PARENT/.bird-uboot-fixed-read-path.XXXXXX") || fail 'could not create staging directory'
cleanup() {
	case "$WORK" in "$OUTPUT_PARENT"/.bird-uboot-fixed-read-path.*) rm -rf -- "$WORK" ;; *) fail 'unsafe staging cleanup' ;; esac
}
...
cp /input/fixed.defconfig configs/'"$BOARD_CONFIG"'
...
for PASS in a b; do
	printf 'Building isolated fixed-read-path pass: %s\n' "$PASS"
...
cleanup
printf 'Verified non-deploying fixed-read-path U-Boot authority: %s\n' "$OUTPUT"
