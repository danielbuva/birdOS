...
grep -Fq 'source-kernel-irq-buttons-lz4.tsv' "$UPDATER" || {
...
grep -Fq 'source-kernel-irq-buttons-no-raid6-deferred-wifi-lz4.tsv' "$UPDATER" || {
	printf '%s\n' 'deferred-Wi-Fi kernel authority is missing from updater input verification' >&2
	exit 1
}
