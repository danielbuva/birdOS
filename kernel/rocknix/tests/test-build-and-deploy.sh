...
if "$COMMAND" --profile --single-gpio-read-kernel \
	>"$TMP/single-profile.out" 2>"$TMP/single-profile.err"; then
...
if "$COMMAND" --profile --changed-input-sync-kernel \
	>"$TMP/changed-sync-profile.out" 2>"$TMP/changed-sync-profile.err"; then
	fail 'changed-input-sync kernel unexpectedly accepted profile mode'
fi
grep -q -- 'source-kernel authorities require --release' "$TMP/changed-sync-profile.err"
if "$COMMAND" --profile --fixed-gpio-fastpath-kernel \
	>"$TMP/fixed-gpio-profile.out" 2>"$TMP/fixed-gpio-profile.err"; then
...
if "$COMMAND" --profile --irq-buttons-lz4-kernel \
	>"$TMP/irq-buttons-lz4-profile.out" 2>"$TMP/irq-buttons-lz4-profile.err"; then
...
if "$COMMAND" --release --builtin-input-kernel --single-gpio-read-kernel \
	>"$TMP/single-authority.out" 2>"$TMP/single-authority.err"; then
...
if "$COMMAND" --release --single-input-sync-kernel --changed-input-sync-kernel \
	>"$TMP/changed-sync-authority.out" 2>"$TMP/changed-sync-authority.err"; then
	fail 'multiple input-sync authorities unexpectedly accepted'
fi
grep -q -- 'choose only one source-kernel authority' "$TMP/changed-sync-authority.err"
if "$COMMAND" --release --changed-input-sync-kernel --fixed-gpio-fastpath-kernel \
	>"$TMP/fixed-gpio-authority.out" 2>"$TMP/fixed-gpio-authority.err"; then
...
if "$COMMAND" --release --irq-buttons-kernel --irq-buttons-lz4-kernel \
	>"$TMP/irq-buttons-lz4-authority.out" 2>"$TMP/irq-buttons-lz4-authority.err"; then
