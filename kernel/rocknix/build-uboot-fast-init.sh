#!/bin/sh
# Reproduce the RG34XX-SP fixed successful-path U-Boot candidate twice.  This
...
OUTPUT=${OUTPUT:-$ROOT/kernel/work/bird-uboot-fast-init-20260701}
BASE=${BASE:-$ROOT/kernel/work/bird-uboot-no-heap-clear-20260701}
...
BASE_AUTHORITY_SHA=cc8a83dcda26c6067ac603bbe31497be79e9836667a5e789c0ded423fe6178de
...
BASE_VERIFIER="$ROOT/kernel/rocknix/verify-uboot-no-heap-clear-build.py"
...
for INPUT in "$FIT_TIMESTAMP_PATCHER" "$ENV_TRANSFORM" "$DIRECT_TRANSFORM" \
	"$NO_CLEAR_TRANSFORM" "$FAST_TRANSFORM" "$VERIFIER" "$BASE_VERIFIER"; do
	require_file "$INPUT"
done
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/bird-uboot-fast-init.XXXXXX")
cleanup() {
	rm -rf "$WORK"
}
...
case "$1" in fast-init-a|fast-init-b) ;; *) exit 2 ;; esac
...
cp /input/fast.defconfig "configs/'"$DEFCONFIG"'"
TOOLCHAIN=/work/build.ROCKNIX-H700.aarch64/toolchain
...
for BUILD_NAME in fast-init-a fast-init-b; do
...
"$PYTHON" "$VERIFIER" \
	--build-a "$WORK/output/fast-init-a" \
	--build-b "$WORK/output/fast-init-b" \
	--base-authority "$BASE" \
	--bl31 "$BL31" \
...
printf 'Verified non-deploying fast-init U-Boot candidate built:\n  %s\n' "$OUTPUT"
