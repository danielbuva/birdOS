#!/bin/sh
# Reproduce the RG34XX-SP in-place initrd/FDT handoff U-Boot candidate twice. This
# is a host artifact builder with no deployment mode; it never opens media.
...
OUTPUT=${OUTPUT:-$ROOT/kernel/work/bird-uboot-inplace-handoff-20260701}
BASE=${BASE:-$ROOT/kernel/work/bird-uboot-fast-init-20260701}
...
BASE_AUTHORITY_SHA=9953516faf5a0c653fb3e779e28f2b3580fb91f3e8d6596dd15061e2ceca2f27
...
BASE_VERIFIER="$ROOT/kernel/rocknix/verify-uboot-fast-init-build.py"
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/bird-uboot-inplace-handoff.XXXXXX")
cleanup() {
	rm -rf "$WORK"
}
...
case "$1" in inplace-handoff-a|inplace-handoff-b) ;; *) exit 2 ;; esac
...
cp /input/inplace.defconfig "configs/'"$DEFCONFIG"'"
...
for BUILD_NAME in inplace-handoff-a inplace-handoff-b; do
	...
	"$PYTHON" "$VERIFIER" \
	--build-a "$WORK/output/inplace-handoff-a" \
	--build-b "$WORK/output/inplace-handoff-b" \
	--base-authority "$BASE" \
	--bl31 "$BL31" \
	--toolchain-authority "$TOOLCHAIN_AUTHORITY" \
	--environment "$WORK/input/$INPLACE_ENV" \
	--output "$WORK/publish"

mkdir -p "$(dirname "$OUTPUT")"
mv "$WORK/publish" "$OUTPUT"
printf 'Verified non-deploying in-place-handoff U-Boot candidate built:\n  %s\n' "$OUTPUT"
