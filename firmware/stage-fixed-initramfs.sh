...
CANDIDATE="$BUILD/bird-boot-fixed-initramfs.img"
...
sh -n "$ROOT/launcher/S03birdlauncher"

mkdir -p "$WORK_DIR" "$LAUNCHER_DIR"
COPYFILE_DISABLE=1 cp -f "$CANDIDATE" "$WORK_DIR/.bird-boot-fixed-initramfs.new"
COPYFILE_DISABLE=1 cp -f "$CHECKSUM" "$WORK_DIR/.bird-boot-fixed-initramfs.sha256.new"
COPYFILE_DISABLE=1 cp -f "$ROOT/firmware/device-install-fixed-initramfs.sh" \
	"$CARD/MUOS/init/.93-install-fixed-initramfs.new"
COPYFILE_DISABLE=1 cp -f "$ROOT/launcher/S03birdlauncher" \
	"$LAUNCHER_DIR/.S03birdlauncher.fixed-init-new"
COPYFILE_DISABLE=1 cp -f "$ROOT/launcher/bird-launcher.o" \
	"$LAUNCHER_DIR/.bird-launcher.fixed-init-new"
COPYFILE_DISABLE=1 cp -f "$ROOT/launcher/catalog.revision" \
	"$LAUNCHER_DIR/.catalog.fixed-init-new"

[ "$(shasum -a 256 "$WORK_DIR/.bird-boot-fixed-initramfs.new" | awk '{print $1}')" = \
	"$EXPECTED_SHA" ] || {
	printf 'error: staged candidate verification failed\n' >&2
	exit 1
}
cmp "$ROOT/launcher/S03birdlauncher" \
	"$LAUNCHER_DIR/.S03birdlauncher.fixed-init-new"
cmp "$ROOT/launcher/bird-launcher.o" \
	"$LAUNCHER_DIR/.bird-launcher.fixed-init-new"
cmp "$ROOT/launcher/catalog.revision" \
	"$LAUNCHER_DIR/.catalog.fixed-init-new"

mv -f "$WORK_DIR/.bird-boot-fixed-initramfs.new" \
	"$WORK_DIR/bird-boot-fixed-initramfs.img"
mv -f "$WORK_DIR/.bird-boot-fixed-initramfs.sha256.new" \
	"$WORK_DIR/bird-boot-fixed-initramfs.sha256"
mv -f "$LAUNCHER_DIR/.S03birdlauncher.fixed-init-new" \
	"$LAUNCHER_DIR/S03birdlauncher"
mv -f "$LAUNCHER_DIR/.bird-launcher.fixed-init-new" \
	"$LAUNCHER_DIR/bird-launcher.o"
...
chmod 755 "$INSTALLER" "$LAUNCHER_DIR/S03birdlauncher"
sync
