...
CANDIDATE="$BUILD/dani-boot-static-pid1.img"
...
mkdir -p "$WORK_DIR"
COPYFILE_DISABLE=1 cp -f "$CANDIDATE" "$WORK_DIR/.dani-boot-static-pid1.new"
COPYFILE_DISABLE=1 cp -f "$CHECKSUM" "$WORK_DIR/.dani-boot-static-pid1.sha256.new"
...
[ "$(shasum -a 256 "$WORK_DIR/.dani-boot-static-pid1.new" | awk '{print $1}')" = \
	"$EXPECTED_SHA" ] || {
...
mv -f "$WORK_DIR/.dani-boot-static-pid1.new" \
	"$WORK_DIR/dani-boot-static-pid1.img"
mv -f "$WORK_DIR/.dani-boot-static-pid1.sha256.new" \
	"$WORK_DIR/dani-boot-static-pid1.sha256"
