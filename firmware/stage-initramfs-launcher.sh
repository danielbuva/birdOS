...
CANDIDATE="$BUILD/dani-boot-initramfs-launcher.img"
...
mkdir -p "$WORK_DIR"
COPYFILE_DISABLE=1 cp -f "$CANDIDATE" "$WORK_DIR/.dani-boot-initramfs-launcher.new"
COPYFILE_DISABLE=1 cp -f "$CHECKSUM" "$WORK_DIR/.dani-boot-initramfs-launcher.sha256.new"
...
[ "$(shasum -a 256 "$WORK_DIR/.dani-boot-initramfs-launcher.new" | awk '{print $1}')" = \
	"$EXPECTED_SHA" ] || {
	printf 'error: staged candidate verification failed\n' >&2
	exit 1
}

mv -f "$WORK_DIR/.dani-boot-initramfs-launcher.new" \
	"$WORK_DIR/dani-boot-initramfs-launcher.img"
mv -f "$WORK_DIR/.dani-boot-initramfs-launcher.sha256.new" \
	"$WORK_DIR/dani-boot-initramfs-launcher.sha256"
