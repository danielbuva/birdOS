#!/bin/sh
# Guarded deployment of Bird clean-root v5.4. Only p1/KERNEL is replaced.
# The v5.3 kernel is preserved on p6; p5 and user content are untouched.
...
KERNEL=${KERNEL:-$ROOT/kernel/work/bird-clean-root-v5-4-kernel-final-a/build/Image}
DTB=${DTB:-$ROOT/kernel/work/bird-clean-root-v5-4-kernel-final-a/build/sun50i-h700-anbernic-rg34xx-sp.dtb}
...
V52_KERNEL_SHA=bb7e320b084a5b643f9c8015ff8986f7fd81a86a7aa76b0789ec3fc9f517646a
V53_KERNEL_SHA=ad60468b880fba88c1fc85eca888fbceefd05eb5d54a9a14f9fe74dd8c0847f9
INTERMEDIATE_KERNEL_SHA=d3eeca9795d64b25d110dfc1bd0fd2becb2d03afa2b2afb1bc63ed107d2dad7b
NEW_KERNEL_SHA=a53a3483731d28d2e96e53def0fba347fa53607aa9fbda8bfb82db677126daef
...
case "$CURRENT" in
	"$V53_KERNEL_SHA" | "$INTERMEDIATE_KERNEL_SHA" | "$NEW_KERNEL_SHA") ;;
	*) fail "card kernel is not clean-root v5.3 or v5.4: $CURRENT" ;;
esac
...
if [ -f "$RECOVERY/KERNEL-v5.3" ]; then
	[ "$(sha256 "$RECOVERY/KERNEL-v5.3")" = "$V53_KERNEL_SHA" ] || \
		fail 'existing v5.3 recovery kernel checksum mismatch'
elif [ "$CURRENT" = "$V53_KERNEL_SHA" ]; then
	COPYFILE_DISABLE=1 cp -f "$BIRD/KERNEL" "$RECOVERY/.KERNEL-v5.3.new"
	[ "$(sha256 "$RECOVERY/.KERNEL-v5.3.new")" = "$V53_KERNEL_SHA" ] || \
		fail 'v5.3 recovery kernel copy failed verification'
	mv -f "$RECOVERY/.KERNEL-v5.3.new" "$RECOVERY/KERNEL-v5.3"
else
	fail 'v5.3 recovery kernel missing on already-updated card'
fi

COPYFILE_DISABLE=1 cp -f "$KERNEL" "$BIRD/.KERNEL.bird-v5-4.new"
[ "$(sha256 "$BIRD/.KERNEL.bird-v5-4.new")" = "$NEW_KERNEL_SHA" ] || \
	fail 'temporary card kernel failed verification'
mv -f "$BIRD/.KERNEL.bird-v5-4.new" "$BIRD/KERNEL"

xattr -c "$BIRD/KERNEL" "$BIRD/dtb.img" "$BIRD/extlinux/extlinux.conf" \
	"$RECOVERY/KERNEL-v5.3" "$RECOVERY/KERNEL-v5.2" \
	"$RECOVERY/KERNEL-v5.1" "$RECOVERY/KERNEL-v5.0" \
	"$RECOVERY/KERNEL-v4.5" 2>/dev/null || :
rm -f "$BIRD/._KERNEL" "$BIRD/._dtb.img" \
	"$BIRD/extlinux/._extlinux.conf" \
	"$RECOVERY/._KERNEL-v5.3" "$RECOVERY/._KERNEL-v5.2" \
	"$RECOVERY/._KERNEL-v5.1" "$RECOVERY/._KERNEL-v5.0" \
	"$RECOVERY/._KERNEL-v4.5"
sync
[ "$(sha256 "$BIRD/KERNEL")" = "$NEW_KERNEL_SHA" ] || \
	fail 'installed clean-root kernel failed verification'

printf 'Bird clean-root v5.4 staged on /dev/%s.\n' "$WHOLE"
printf 'p1/KERNEL changed; p5 and the p6 library are untouched.\n'
printf 'The v5.3 kernel is preserved at MUOS/Bird/recovery/KERNEL-v5.3.\n'
printf 'Kernel: %s\n' "$NEW_KERNEL_SHA"
printf 'Test: boot/menu; Dreamcast; DS/layouts; PSP; Balatro audio; Stardew; MP3; visible movie/controls; Select+Start; volume; brightness; button and lid suspend/wake; shutdown.\n'
