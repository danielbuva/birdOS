#!/bin/sh
# Update only the mounted FAT boot payload after the first Bird/source-kernel
# hardware gate. The root and data partitions are deliberately untouched.
...
OLD_KERNEL_SHA=f93cdd0008f9dae05be9192d9360fd097213cb377163d3d19cd58624c4bd5c31
NEW_KERNEL_SHA=0fa4d5d2d30423302bb83be86761465799b21f0fda396544e09c3e700789f597
...
case "$CURRENT_KERNEL_SHA" in
	...
	*) fail "card kernel is not an accepted source-kernel candidate: $CURRENT_KERNEL_SHA" ;;
esac

mkdir -p "$BIRD/extlinux"
COPYFILE_DISABLE=1 cp -f "$KERNEL" "$BIRD/.KERNEL.bird-v2.new"
COPYFILE_DISABLE=1 cp -f "$CONFIG" "$BIRD/extlinux/.extlinux.conf.bird-v2.new"
[ "$(shasum -a 256 "$BIRD/.KERNEL.bird-v2.new" | awk '{print $1}')" = \
	"$NEW_KERNEL_SHA" ] || fail 'temporary card kernel verification failed'
cmp "$CONFIG" "$BIRD/extlinux/.extlinux.conf.bird-v2.new" || \
	fail 'temporary extlinux verification failed'

mv -f "$BIRD/.KERNEL.bird-v2.new" "$BIRD/KERNEL"
mv -f "$BIRD/extlinux/.extlinux.conf.bird-v2.new" \
...
printf 'Bird/source-kernel compatibility v2 staged on /dev/%s.\n' "$BIRD_WHOLE"
printf 'Only p1 was updated; p5 root and p6 data were not written.\n'
printf 'Kernel: %s\n' "$NEW_KERNEL_SHA"
printf 'Next test: terminal ownership, D-pad/A/B, suspend/wake, audio and brightness.\n'
