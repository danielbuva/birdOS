#!/bin/sh
# Guarded deployment of Bird clean-root v5.1. Only p1/KERNEL is replaced.
# The accepted v5.0 kernel is preserved on p6. P5 and the content library are
...
KERNEL=${KERNEL:-$ROOT/kernel/work/bird-clean-root-v5-1-kernel-release-a/build/Image}
DTB=${DTB:-$ROOT/kernel/work/bird-clean-root-v5-1-kernel-release-a/build/sun50i-h700-anbernic-rg34xx-sp.dtb}
RUNTIME=$DATA/MUOS/runtime/ROCKNIX-SYSTEM
RECOVERY=$DATA/MUOS/Bird/recovery

OLD_KERNEL_SHA=b585d2b59ffd735e16cfefe1fcafeaf4d2d56831d0d4bd937819a87440a4be64
NEW_KERNEL_SHA=5f1238b5f3010f88ac142505097064264c2a6090d0e19a809a0c1a97da6bde5c
...
case "$CURRENT" in
	...
	*) fail "card kernel is not clean-root v5.0 or v5.1: $CURRENT" ;;
esac

mkdir -p "$RECOVERY"
if [ -f "$RECOVERY/KERNEL-v5.0" ]; then
	[ "$(sha256 "$RECOVERY/KERNEL-v5.0")" = "$OLD_KERNEL_SHA" ] || \
		fail 'existing v5.0 recovery kernel checksum mismatch'
elif [ "$CURRENT" = "$OLD_KERNEL_SHA" ]; then
	COPYFILE_DISABLE=1 cp -f "$BIRD/KERNEL" "$RECOVERY/.KERNEL-v5.0.new"
	[ "$(sha256 "$RECOVERY/.KERNEL-v5.0.new")" = "$OLD_KERNEL_SHA" ] || \
		fail 'v5.0 recovery kernel copy failed verification'
	mv -f "$RECOVERY/.KERNEL-v5.0.new" "$RECOVERY/KERNEL-v5.0"
else
	fail 'v5.0 recovery kernel missing on already-updated card'
fi

COPYFILE_DISABLE=1 cp -f "$KERNEL" "$BIRD/.KERNEL.bird-v5-1.new"
[ "$(sha256 "$BIRD/.KERNEL.bird-v5-1.new")" = "$NEW_KERNEL_SHA" ] || \
	fail 'temporary card kernel failed verification'
mv -f "$BIRD/.KERNEL.bird-v5-1.new" "$BIRD/KERNEL"
sync
[ "$(sha256 "$BIRD/KERNEL")" = "$NEW_KERNEL_SHA" ] || \
	fail 'installed clean-root kernel failed verification'

printf 'Bird clean-root v5.1 staged on /dev/%s.\n' "$WHOLE"
printf 'p1/KERNEL changed; p5 and the p6 library are untouched.\n'
printf 'The v5.0 kernel is preserved at MUOS/Bird/recovery/KERNEL-v5.0.\n'
printf 'Kernel: %s\n' "$NEW_KERNEL_SHA"
printf 'Test: game, movie, MP3, Select+Start, system volume, brightness, shutdown.\n'
