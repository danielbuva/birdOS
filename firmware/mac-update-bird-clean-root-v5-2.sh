#!/bin/sh
# Guarded deployment of Bird clean-root v5.2. Only p1/KERNEL is replaced.
# The accepted v5.1 kernel is preserved on p6. P5 and the content library are
...
OLD_KERNEL_SHA=5f1238b5f3010f88ac142505097064264c2a6090d0e19a809a0c1a97da6bde5c
NEW_KERNEL_SHA=bb7e320b084a5b643f9c8015ff8986f7fd81a86a7aa76b0789ec3fc9f517646a
...
case "$CURRENT" in
	...
	*) fail "card kernel is not clean-root v5.1 or v5.2: $CURRENT" ;;
esac

mkdir -p "$RECOVERY"
[ -f "$RECOVERY/KERNEL-v5.0" ] || fail 'v5.0 recovery kernel missing'
[ "$(sha256 "$RECOVERY/KERNEL-v5.0")" = "$V50_KERNEL_SHA" ] || \
	fail 'existing v5.0 recovery kernel checksum mismatch'

if [ -f "$RECOVERY/KERNEL-v5.1" ]; then
	[ "$(sha256 "$RECOVERY/KERNEL-v5.1")" = "$OLD_KERNEL_SHA" ] || \
		fail 'existing v5.1 recovery kernel checksum mismatch'
elif [ "$CURRENT" = "$OLD_KERNEL_SHA" ]; then
	COPYFILE_DISABLE=1 cp -f "$BIRD/KERNEL" "$RECOVERY/.KERNEL-v5.1.new"
	[ "$(sha256 "$RECOVERY/.KERNEL-v5.1.new")" = "$OLD_KERNEL_SHA" ] || \
		fail 'v5.1 recovery kernel copy failed verification'
	mv -f "$RECOVERY/.KERNEL-v5.1.new" "$RECOVERY/KERNEL-v5.1"
else
	fail 'v5.1 recovery kernel missing on already-updated card'
fi

COPYFILE_DISABLE=1 cp -f "$KERNEL" "$BIRD/.KERNEL.bird-v5-2.new"
[ "$(sha256 "$BIRD/.KERNEL.bird-v5-2.new")" = "$NEW_KERNEL_SHA" ] || \
	fail 'temporary card kernel failed verification'
mv -f "$BIRD/.KERNEL.bird-v5-2.new" "$BIRD/KERNEL"

# macOS may synthesize com.apple.provenance AppleDouble sidecars even when
# COPYFILE_DISABLE is set. They are never part of Bird's boot contract.
...
printf 'Bird clean-root v5.2 staged on /dev/%s.\n' "$WHOLE"
printf 'p1/KERNEL changed; p5 and the p6 library are untouched.\n'
printf 'The v5.1 kernel is preserved at MUOS/Bird/recovery/KERNEL-v5.1.\n'
printf 'Kernel: %s\n' "$NEW_KERNEL_SHA"
printf 'Test: normal-speed game/Dreamcast with audio; MP3; immediately advancing movie with audio; 1x movie speed; Select+Start; system volume; brightness; shutdown.\n'
