...
BASE=${1:-$ROOT/firmware/work/trimmed-initramfs/bird-boot-trimmed-initramfs.img}
OUTPUT=${2:-$ROOT/firmware/work/direct-handoff-initramfs}

"$ROOT/firmware/build-trimmed-initramfs.sh" "$BASE" "$OUTPUT"

CANDIDATE="$OUTPUT/bird-boot-trimmed-initramfs.img"
strings "$OUTPUT/verify/ramdisk/init" | grep -q 'direct-handoff-static-pid1'
strings "$OUTPUT/verify/ramdisk/init" | grep -q 'bird-fsck-clean-skip'
